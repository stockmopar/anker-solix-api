#!/usr/bin/env python3
"""Example: Basic F3800(P) device control via MQTT."""
...
from api.mqtt_f3800 import SolixMqttDeviceF3800  # pylint: disable=no-name-in-module
import common
...
async def main():
    ...
    async with ClientSession() as websession:
        ...
        for sn, device in myapi.devices.items():
            if device.get("device_pn") in ["A1790","A1790P"]:
                device_sn = sn
                CONSOLE.info(f"Found {device.get("device_pn")} device: {sn}")
                break

        if not device_sn:
            CONSOLE.info("No A1790(P) device found")
            return
